...
interface ParticipantCommunication {
  id: string;
...
interface Props {
  ...
  visible?: boolean;
  autoLoad?: boolean;
  /** Teléfono del participante (nacional o con lada) para el link de WhatsApp. */
  phone?: string | null;
  /** País del participante (ISO-2 o texto libre) para resolver la lada. */
  country?: string | null;
}
...
// Link al chat real de WhatsApp con el participante (null → botón oculto).
const whatsappLink = computed(() => buildWhatsAppChatLink(props.phone, props.country));

// Load history function
const loadHistory = async (loadMore = false) => {
  if (!props.participantId || !props.retreatId) return;

  localLoading.value = true;
  emit('loading-changed', true);

  try {
    await fetchParticipantCommunications(
      props.participantId,
      {
...
// Clear communications when participant or retreat changes
watch(
  [() => props.participantId, () => props.retreatId],
  () => {
    clearCommunications();
  }
);
...
// Initialize on mount
onMounted(() => {
  if (props.participantId && props.retreatId && props.visible && props.autoLoad) {
    loadHistory();
  }
});
</script>
