...
vi.mock('pinia', () => ({
  ...
  storeToRefs: vi.fn((store) => Object.keys(store).reduce((acc: Record<string, any>, key) => {
...
expect.extend({
  ...
  toHaveAttribute(received, name, value) {
    const pass = received && received.hasAttribute(name) &&
  ...
  toHaveStyle(received, styles) {
    const pass = received && Object.entries(styles).every(([prop, val]) =>
      received.style[prop as any] === val
    );
    return {
      message: () =>
        pass
