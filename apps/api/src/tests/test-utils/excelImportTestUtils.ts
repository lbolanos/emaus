...
export class ExcelImportTestUtils {
  ...
  static validateImportResult(result: any): {
    ...
    const errors: string[] = [];
    const requiredFields = ['importedCount', 'updatedCount', 'skippedCount', 'tablesCreated', 'bedsCreated', 'paymentsCreated'];
  ...
  static async verifyParticipantImport(
    ...
    for (const expected of expectedData) {
      ...
      if (expected.nombre && actual.firstName !== expected.nombre.trim()) {
        errors.push(`FirstName mismatch for ${expected.email}: expected "${expected.nombre}", got "${actual.firstName}"`);
      }

      if (expected.apellidos && actual.lastName !== expected.apellidos.trim()) {
        errors.push(`LastName mismatch for ${expected.email}: expected "${expected.apellidos}", got "${actual.lastName}"`);
      }

      if (expected.email && actual.email !== expected.email.trim()) {
        errors.push(`Email mismatch for ${expected.email}: expected "${expected.email}", got "${actual.email}"`);
      }

      // Verify retreat assignment
      if (actual.retreatId !== retreatId) {
        errors.push(`RetreatId mismatch for ${expected.email}: expected "${retreatId}", got "${actual.retreatId}"`);
      }
      ...
      if (expected.anio && expected.mes && expected.dia) {
        ...
        if (actual.birthDate && actual.birthDate.toDateString() !== expectedDate.toDateString()) {
          errors.push(`BirthDate mismatch for ${expected.email}: expected "${expectedDate.toDateString()}", got "${actual.birthDate.toDateString()}"`);
        }
      }
      ...
      if (expectedType && actual.type !== expectedType) {
        errors.push(`Type mismatch for ${expected.email}: expected "${expectedType}", got "${actual.type}"`);
      }

      // Verify cancellation status
      const expectedCancelled = expected.cancelado?.trim() === 'S';
      if (actual.isCancelled !== expectedCancelled) {
        errors.push(`Cancellation status mismatch for ${expected.email}: expected "${expectedCancelled}", got "${actual.isCancelled}"`);
      }
    }
  ...
  static async verifyTableAssignments(
    ...
    for (const expected of expectedAssignments) {
      ...
      if (!participant.tableMesa) {
        errors.push(`Participant ${expected.email} has no table assigned, expected "${expected.tableName}"`);
        continue;
      }

      if (participant.tableMesa.name !== expected.tableName) {
        errors.push(`Table assignment mismatch for ${expected.email}: expected "${expected.tableName}", got "${participant.tableMesa.name}"`);
      }
    }
  ...
  static async verifyBedAssignments(
    ...
    for (const expected of expectedAssignments) {
      ...
      if (!participant.retreatBed) {
        errors.push(`Participant ${expected.email} has no bed assigned, expected room "${expected.roomNumber}"`);
        continue;
      }

      if (participant.retreatBed.roomNumber !== expected.roomNumber) {
        errors.push(`Bed assignment mismatch for ${expected.email}: expected room "${expected.roomNumber}", got "${participant.retreatBed.roomNumber}"`);
      }
    }
    ...
    for (const group of expectedGroups) {
      ...
      if (group.shouldHaveSameColor && colors.length > 0) {
        ...
        if (uniqueColors.length > 1) {
          errors.push(`Group [${group.emails.join(', ')}] should have same color but has different colors: ${uniqueColors.join(', ')}`);
        }
      }
    }
  ...
  static async verifyLeadershipAssignments(
    participants: Participant[],
    expectedLeadership: { email: string; expectedRole?: 'lider' | 'colider1' | 'colider2'; tableName: string }[]
  ): Promise<{
    success: boolean;
    errors: string[];
  }> {
    ...
    for (const expected of expectedLeadership) {
      ...
      if (!participant) {
        errors.push(`Participant ${expected.email} not found for leadership assignment verification`);
        continue;
      }
      ...
      if (!table) {
        errors.push(`Table "${expected.tableName}" not found for leadership assignment verification`);
        continue;
      }
      ...
      if (expected.expectedRole && actualRole !== expected.expectedRole) {
        errors.push(`Leadership role mismatch for ${expected.email} in table "${expected.tableName}": expected "${expected.expectedRole}", got "${actualRole}"`);
      }

      if (!expected.expectedRole && actualRole) {
        errors.push(`Participant ${expected.email} has leadership role "${actualRole}" but none expected`);
      }
    }
   ...
   */
  private static getParticipantLeadershipRole(participant: Participant, table: any): 'lider' | 'colider1' | 'colider2' | null {
