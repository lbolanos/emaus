...
async function runExcelImportTests() {
  ...
  try {
    ...
    await execAsync('pnpm add --filter api @types/jest jest ts-jest @types/supertest supertest jsonwebtoken @types/jsonwebtoken', { stdio: 'pipe' });
...
async function runSpecificTests(testPatterns: string[]) {
  ...
  const configPath = path.join(dirname(fileURLToPath(import.meta.url)), 'jest.specific.config.json');
