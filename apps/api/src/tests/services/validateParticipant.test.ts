...
describe("validateParticipant service", () => {
  ...
  it("should return error when retreat endDate is in the past", async () => {
  ...
  it("should not flag retreat as past when endDate is in the future", async () => {
    mockFindOne.mockResolvedValue({
      id: "retreat-1",
      isPublic: true,
      endDate: "2099-12-31",
  ...
  it("should warn when walker capacity is reached", async () => {
    ...
    expect(result.valid).toBe(true);
    expect(
      result.warnings.some((w) => w.includes("capacidad máxima de caminantes")),
    ).toBe(true);
  });
  ...
  it("should warn when server capacity is reached", async () => {
    ...
    expect(result.valid).toBe(true);
    expect(
      result.warnings.some((w) => w.includes("capacidad máxima de servidores")),
    ).toBe(true);
  });
