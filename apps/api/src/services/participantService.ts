...
import { getCommunityTimezone } from "./communityService";
...
export const findNextMeetingForParticipant = async (
    ...
    if (recurringTemplate) {
      ...
      while (cursor.getTime() < now.getTime() && safety-- > 0) {
        const next = calculateNextOccurrence(
          ...
          getCommunityTimezone(recurringTemplate.community),
        );
        if (!next) {
          cursor = null as any;
          break;
  ...
  // Usar el timezone IANA de la community (inferido desde lat/lon al crear/
  // editar la community via `inferTimezoneFromCoords`). Fallback a CDMX si la
  // community no lo tiene seteado todavía. Sin esto, en el server Lightsail
  // (UTC) un meeting a las 19:00 hora local rendereaba como "1:45 a.m." UTC.
  const communityTz =
    (nextMeeting as any).community?.timezone || "America/Mexico_City";
...
export const findParticipantById = async (
  ...
  // Paz y salvo per-retiro + overlay de campos por retiro: extraído a
  // participantRetreatHydration para que el motor de secuencias lo reúse
  // (aquí no puede importarlo: import circular con messageSequenceService).
  if (overlayRetreatId) {
    await hydrateParticipantRetreatContext(participant, overlayRetreatId);
  }
