...
export class MigrateAndRemoveLegacyPayments20251024000000 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		console.log(
			'🔄 Starting migration: Migrate legacy payment amounts and remove legacy fields...',
		);
		...
		if (hasPaymentAmount && hasPaymentDate) {
			...
			for (const participant of participantsWithPayment) {
				...
				if (existingPayments[0].count === 0) {
					...
					console.log(
						`✅ Created payment record for participant ${participant.id}: $${participant.paymentAmount}`,
					);
				} else {
					console.log(
						`⏭️  Skipping participant ${participant.id} - already has ${existingPayments[0].count} payment records`,
					);
				}
			}
			...
			console.log(
				'🗑️ Removing legacy paymentAmount and paymentDate fields from participants table...',
			);
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		console.log(
			'↩️ Rolling back: Re-adding legacy payment fields and removing migrated payment records...',
		);
		...
		console.log(
			'✅ Rollback completed - legacy payment fields restored and migrated records removed',
		);
	}
}
