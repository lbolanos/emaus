...
export class FixBedAssignmentInconsistencies20251025000000 implements MigrationInterface {
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		inconsistencies.forEach((inc: any) => {
			console.log(
				`  - ${inc.firstName} ${inc.lastName}: ${inc.status} (Room ${inc.roomNumber}, Bed ${inc.bedNumber})`,
			);
		});
		...
		const brokenReferences = inconsistencies.filter(
			(inc: any) => inc.status === 'bidirectional_reference_broken',
		);
		console.log(`🔧 Fixing ${brokenReferences.length} broken bidirectional references...`);

		for (const inconsistency of brokenReferences) {
			console.log(
				`  - Fixing ${inconsistency.firstName} ${inconsistency.lastName}: Setting retreatBedId to NULL (bed assigned to ${inconsistency.bed_participant_id})`,
			);
		...
		const missingBackReferences = inconsistencies.filter(
			(inc: any) => inc.status === 'bed_points_to_participant_but_no_back_reference',
		);
		console.log(`🔧 Fixing ${missingBackReferences.length} missing back references...`);

		for (const inconsistency of missingBackReferences) {
			console.log(
				`  - Fixing ${inconsistency.firstName} ${inconsistency.lastName}: Setting retreatBedId to ${inconsistency.retreat_bed_id}`,
			);
		...
		const nonExistentBeds = inconsistencies.filter(
			(inc: any) => inc.status === 'participant_points_to_nonexistent_bed',
		);
		console.log(`🔧 Fixing ${nonExistentBeds.length} references to non-existent beds...`);

		for (const inconsistency of nonExistentBeds) {
			console.log(
				`  - Fixing ${inconsistency.firstName} ${inconsistency.lastName}: Setting retreatBedId to NULL (bed ${inconsistency.participant_bed_id} doesn't exist)`,
			);
		...
		console.log(
			`✅ Verification complete. Remaining inconsistencies: ${remainingInconsistencies.length}`,
		);
		...
		try {
			...
			console.log(
				'  ⚠️  Unique constraint already exists or could not be created:',
				error?.message || error,
			);
		}
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		console.log(
			'⚠️  Note: Data changes made by this migration cannot be automatically rolled back.',
		);
